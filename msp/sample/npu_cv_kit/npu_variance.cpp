...
int main(int argc, char* argv[]) {
    ...
    AX_NPU_CV_Matrix2D result_matrix_2d = create_matrix_by_size(1, 1, result_data_type);

    AX_NPU_SDK_EX_MODEL_TYPE_T virtual_npu_mode_type = get_npu_mode_type(args.get<std::string>("mode-type"));

    auto time_start = std::chrono::system_clock::now();
    for (int i = 0; i < nRepeat; ++i) {
        assert(0 == AX_NPU_CV_Variance(virtual_npu_mode_type, &a_matrix_2d, &result_matrix_2d));
        if(nRepeat > 1 && i == 0){
    ...
    printf("**************************************************\n");
     ...
     if(!output.empty()){
        ...
        if (stat(output.c_str(), &info) != 0 || !(info.st_mode & S_IFDIR)) {
            ...
            char fname[256];
            sprintf(fname, "%s/npu_variance.row_%d.column_%d.npu_mode_%d.%s",
                output.c_str(),
                nRow, nColumn,
    ...
    if (bCheck) {
        ...
        uint8_t *a_data = (uint8_t *)a_matrix_2d.pVir;
        float average = 0.0;
        printf("gt data---------------------->\n");
        for (int r = 0; r < nRow; ++r) {
            for (int c = 0; c < nColumn; ++c) {
                average += a_data[r*nColumn+c];
            }
        }
        average /= (float)(nRow*nColumn);
        printf("average = %f\n", average);
        for (int r = 0; r < nRow; ++r) {
            for (int c = 0; c < nColumn; ++c) {
                float d = (float)a_data[r*nColumn+c]-average;
                gt_data += d*d;
            }
        }

        gt_data /= (float)(nRow*nColumn);
        printf("gt = %8.3f\n", gt_data);
        printf("**************************************************\n");
        printf("**************************************************\n");

        printf("dump variance results:\n");
        float* result_data = (float*)result_matrix_2d.pVir;
        printf("%8.3f", result_data[0]);

        // Check GT with result
        bool match_gt = true;
        if(result_data[0] - gt_data > 1e-3*result_data[0]) match_gt = false;

        if (match_gt) {
            printf("The variance result match with GT!\n");
        } else {
            printf("The variance result mismatch with GT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        }
