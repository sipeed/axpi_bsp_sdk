...
int main(int argc, char* argv[]) {
    ...
    // example:
    // LD_LIBRARY_PATH=./lib:$LD_LIBRARY_PATH ./bin/npu_alpha_blending_with_mask.ax620a --mode disable --mode-type disable --color-space nv12 --bg blend/ori_R.yuv --bg-width 3840 --bg-height 1920  --fg blend/ori_L.yuv --fg-width 3840 --fg-height 1920 --mask blend/mask_y.bin --mask-width 3840 --mask-height 1920 --output .
    ...
    AX_NPU_SDK_EX_MODEL_TYPE_T virtual_npu_mode_type = get_npu_mode_type(args.get<std::string>("mode-type"));
    {
        ...
        for (int i = 0; i < test_count; ++i) {
            assert(0 == AX_NPU_CV_AlphaBlendingWithMask(&tContext, virtual_npu_mode_type, bg, fg, mask, offset_w, offset_h));
        }
        ...
        if (!out_dir.empty()) {
            ...
            // dump result
            char fn[256];
            sprintf(fn, "%s/alpha_blending_with_mask_bg_%dx%d_fg_%dx%d_mask_%dx%d_dtype_%d.bin", out_dir.c_str(), bg->tStride.nW, bg->nHeight, fg->nWidth, fg->nHeight, mask->nWidth, mask->nHeight, (int)bg->eDtype);
            dump_image_to_file(fn, bg);
        }
    }
