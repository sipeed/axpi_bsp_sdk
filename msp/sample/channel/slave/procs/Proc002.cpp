...
AX_BOOL CProc002::RunProcess(AX_U8 nCmd, AX_U8 nSubCmd, AX_U8* pData, AX_U32 nSize) {
  ...
  case CMD_003: // slave recv data
    ...
    break;
  case CMD_004: // slave send data
    {
      AX_S32 ret = AX_Channel_Slave_WriteData(nCmd, nSubCmd, (AX_U8*)&m_data, sizeof(CASE_L_PACKET_T));
      if (0 != ret) {
        printf("cmd[%d]: slave write back data failed, ret: 0x%x\n", nCmd, ret);
      }
    }
